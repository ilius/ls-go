...
static inline int order (unsigned char c) {
	if (isdigit (c))
		return 0;
	else if (isalpha (c))
		return c;
...
*/
static int verrevcmp (const char *s1, size_t s1_len, const char *s2, size_t s2_len) {
	size_t s1_pos = 0;
	size_t s2_pos = 0;
...
int filevercmp (const char *s1, const char *s2) {
	...
	if ((s1_suffix || s2_suffix) && (s1_len == s2_len) && 0 == strncmp (s1, s2, s1_len)) {
		s1_len = s1_pos - s1;
		s2_len = s2_pos - s2;
	}
